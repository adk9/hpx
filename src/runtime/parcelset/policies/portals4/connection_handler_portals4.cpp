...
namespace hpx { namespace parcelset { namespace policies { namespace portals4
{
    std::vector<std::string> connection_handler::runtime_configuration()
    {
        ...
        lines +=
            "buffer_size = ${HPX_PARCEL_PORTALS4_BUFFER_SIZE:65536}"
            ;
    ...
    bool connection_handler::run()
    {
             ...
             it != end; ++it, ++tried)
        {
            try {
                ...
                // Register Portals4's progress function with the IO
                // service pool
        ...
        if (errors.get_error_count() == tried) {
            ...
            HPX_THROW_EXCEPTION(network_error,
                "portals4::connection_handler::run", errors.get_message());
            return false;
        }
        return true;
    }
    ...
    {
        // TODO: Get the locality name here.
        return "portals4";
    }
        ...
        naming::locality const& l, error_code& ec)
    {
            ...
            try {
                ...
                if (hpx::threads::get_self_ptr()) {
                    this_thread::suspend(hpx::threads::pending,
                        "connection_handler(portals4)::create_connection");
                }
                else {
                ...
                HPX_THROWS_IF(ec, network_error,
                    "portals4::parcelport::get_connection", e.what());
        ...
        if (error) {
            ...
            HPX_THROWS_IF(ec, network_error,
                "portals4::parcelport::get_connection",
